...
def writeEmptySplFile (homeDir):
    ...
    emptySplFilePath = os.path.normcase(os.path.join(homeDir, 'vimfiles/spell/fromtags.utf-8.spl'))
    open (emptySplFilePath, 'wb').write (emptySplFile)
    emptySplFilePath = os.path.normcase(os.path.join(homeDir, 'vimfiles/spell/fromtags.latin1.spl'))
    open (emptySplFilePath, 'wb').write (emptySplFile)
...
def writeThings (homeDir, things):
    ...
    dictFile = os.path.normcase(os.path.join(homeDir, 'vimfiles/spell/fromtags.utf-8.add'))
    writeWordList (things, dictFile)
    dictFile = os.path.normcase(os.path.join(homeDir, 'vimfiles/spell/fromtags.latin1.add'))
    writeWordList (things, dictFile)
    ...
    cmd = 'vim --cmd ' + '":mkspell! %s"' % (dictFile.replace(' ', '\ ')) + ' --cmd ":q"'
    os.system (cmd)
    print 'spl generated.'
...
if __name__ == '__main__':
    ...
    parser.add_option('-t', '--tags', help='Location of the tags file to process')
    parser.add_option('-d', '--doxygen', help='Location of the Doxygen generated XML file')

    (options, args) = parser.parse_args()

    if options.tags is None or options.doxygen is None:
        print('Input arguments error, call with the -h or --help option to get help')
    else:
        myTagsFile = options.tags
        myIndexXml = options.doxygen
        main ()
